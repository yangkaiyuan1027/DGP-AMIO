...
class GAT(nn.Module):
    def __init__(self, in_feats=1,
        ...
        self.GAT11 = GATConv(in_feats, h_feats, heads=heads, dropout=0.2, concat=True, edge_dim=edge_feats)
        self.GAT12 = GATConv(h_feats * heads, 1, heads=1, dropout=0.2, concat=False, edge_dim=edge_feats)

    def forward(self, X, A1, A2, A3, edge_feature, return_alphas=False):
        #         alphas = []
        X = self.GAT11(X, A1, edge_feature)
        X = F.relu(X)
        X = self.dropout(X)


        X = self.GAT12(X, A1, edge_feature)
...
class GAT_without_edge_feature(nn.Module):
    def __init__(self, in_feats=1,
                 h_feats=16,
                 heads=8,
                 edge_feats=2,
                 ...
                 negative_slope=0.2,
                 **kwargs):
        super(GAT_without_edge_feature, self).__init__()
        self.dropout = nn.Dropout(dropout)
        self.ln = Linear(3, 1)

        self.GAT11 = GATConv(in_feats, h_feats, heads=heads, dropout=0.2, concat=True)
        self.GAT12 = GATConv(h_feats * heads, 1, heads=1, dropout=0.2, concat=False)
...
class GCN(nn.Module):
    def __init__(self, in_feats=1,
        ...
        self.GCN1 = GCNConv(in_feats, 300)
        self.GCN2 = GCNConv(300, 100)
        self.ffn = Linear(100,1)

    def forward(self, X, A1, A2, A3, edge_feature):
        X = self.GCN1(X, A1)
        X = F.relu(X)
        X = self.dropout(X)
